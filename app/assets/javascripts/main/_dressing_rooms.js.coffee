...
App.DressingRooms =
  showInit: ->
    this.controller = new this.TileController $("#tiles_container")

  TileController: can.Control
    init: (el, options) ->
      @element.show()
...
#        Routes[route_name + '_path'].apply(null, args)
    ...
    vote: (li, type, success) ->
      ...
      $.ajax
        ...
        success: (json) =>
          if json.success
            if helper.roomInfo.user_owned && !li.hasClass('active')
